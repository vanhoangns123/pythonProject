...
class ExcelPython:
    ...
    def getPriceByProperty(self, propertyName, house_type = 0, road_condition = 0):
        ...
        property_id = self.__getPropertyID(propertyName)
    ...
    def getAreaAverage(self, propertyName, propertyPriceArrCol):
        temp = self.__createTempAveragePriceDf(propertyName)
        # add avarage value & check if valid
        self.__addAreaAveragePrice(temp, propertyName, propertyPriceArrCol)

    def __getPropertyID(self, propertyName):
        switcher = {
            "dat": 1040,
            "nha_o": 1020,
            "can_ho": 1010,
    ...
    def __groupTable(self, propertyName):
    ...
    def __createTempAveragePriceDf(self, propertyName):
        tempGroup = self.__groupTable(propertyName)
        avarage = []
    ...
    def __addAreaAveragePrice(self, averageAreaDf, propertyName, propertyPriceArrCol):
        df = self.df
        data = []
        data_err = []
        group = 0
    ...
    def groupColCustom(self, price, dt, type, ward):
        price = self.df[price]
        dt = self.df[dt]
        df = self.df
        data_table = []
        for i in range(self.numRow):
            p = self.__splitPrice(price[i])
            s = self.__splitSize(dt[i])
            a = round(p / s)
            data_table.append(
                {
                    'total': a,
        ...
        df[ward] = self.df[ward]

        df = df.groupby([type, ward], as_index=False, sort=False).sum()

        avr = []
        for i in range(df.shape[0]):
            av = round(df['total'][i] / df['count'][i])
            avr.append(av)
        df['price'] = avr
    ...
    def groupCol(self, groupCol):
        df = self.df.filter(groupCol, axis=1)
        df = df.groupby(groupCol, as_index=False, sort=False).size()
        return df

    # Giá: 8,799 tỷ 
    # Giá: 700 triệu 
    def __splitPrice(self, price):
        price = price.split(' ')
        m = price[2]
        if(m == 'ngàn'):
            m = 1000
        ...
        result = float(price[1].replace(',','.')) * m
        return result

    #Diện tích: 80 m2
    def __splitSize(self, size):
        size = size.split(' ')
        result = int(size[2].replace('.',''))
        return result
...
excelFile = ExcelPython('alonhadat_haugiang')
excelFile.groupColCustom('price', 'dt', 'type', 'ward')
...
excelFile.export('alonhadat_haugiang')
